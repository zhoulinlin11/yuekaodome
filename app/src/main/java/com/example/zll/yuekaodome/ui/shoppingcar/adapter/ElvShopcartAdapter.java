package com.example.zll.yuekaodome.ui.shoppingcar.adapter;
...
import com.example.zll.yuekaodome.ui.shoppingcar.AddSubView;
import com.example.zll.yuekaodome.ui.shoppingcar.ShopCarPresenter;
...
public class ElvShopcartAdapter extends BaseExpandableListAdapter {
    ...
    private String uid="12879";
    private String token="10401FFDCD75934C322C4A5C81931740";
    ...
    private ShopCarPresenter shopcartPresenter;

    public ElvShopcartAdapter(Context context, List<SellerBean> groupList, List<List<ShopBean.DataBean.ListBean>> childList, ShopCarPresenter shopcartPresenter) {
    ...
    public View getGroupView(final int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = inflater.inflate(R.layout.shopcaritem1, null);
    ...
    public View getChildView(final int groupPosition, int childPosition, boolean isLastChild, View convertView,
        ...
        if (convertView == null) {
            ...
            convertView = inflater.inflate(R.layout.shopcaritem2, null);
        ...
        childViewHolder.addSubView.setAddOnclickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //调用更新购物车的接口即可
                shopcartPresenter.updateCarts("12879", sellerid, pid + "", num + "", isChecked, "10401FFDCD75934C322C4A5C81931740");
            }
        });
    ...
    private void updateProductInSeller() {
        ...
        //获取pid
        ShopBean.DataBean.ListBean listBean = childList.get(groupPosition).get(productIndex);
        int num = listBean.getNum();
        int pid = listBean.getPid();
    ...
    public String[] computeMoneyAndNum() {
        ...
        for (int i = 0; i < groupList.size(); i++) {
            for (int j = 0; j < childList.get(i).size(); j++) {
                //判断商品是否选中
                ShopBean.DataBean.ListBean listBean = childList.get(i).get(j);
    ...
    public List<List<ShopBean.DataBean.ListBean>> getchildList() {
        List<List<ShopBean.DataBean.ListBean>> cList = new ArrayList<>();
        for (int i = 0; i < groupList.size(); i++) {
            List<ShopBean.DataBean.ListBean> l = new ArrayList<>();
            for (int j = 0; j < childList.get(i).size(); j++) {
